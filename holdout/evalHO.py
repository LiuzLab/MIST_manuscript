	...
	for i in trange(len(spots)):
		...
		try:
			...
			pccs_all.append(pearsonr(ori.loc[spot,:].to_numpy(),
					 model_data.loc[spot,:].to_numpy())[0]) # calculate PCC
...
def evalGene(ori, mask, ho, meta, model_data, model_name):
	...
	for i in trange(len(genes)):
		...
		pccs_all.append(pearsonr(ori.loc[:, gene].to_numpy(),
								model_data.loc[:, gene].to_numpy())[0]) # calculate PCC
...
def evalAll(data_folder, model_names, slideonly=True, cvFold=5):
	...
	model_perf_dfs.to_csv(os.path.join(data_folder, "performance/slide_level_results.csv"))
	...
	gene_perf_dfs.to_csv(os.path.join(data_folder, "performance/gene_level_results.csv"))
